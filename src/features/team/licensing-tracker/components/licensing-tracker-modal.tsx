...
import { buildLicensingColumns } from '../licensing-tracker-columns';
import {
...
interface LicensingTrackerModalProps {
  open: boolean;
...
export function LicensingTrackerModal({
  open,
  ownerUserId,
  ownerName,
  onClose,
}: LicensingTrackerModalProps) {
  ...
  const loadRows = useCallback(async (
    ...
    try {
      ...
      addToast({
        type: 'error',
        message: err instanceof Error ? err.message : 'Failed to load licensing tracker.',
      });
    } finally {
  ...
  const handlePatchField = useCallback(async (
    ...
    try {
      ...
      addToast({
        type: 'error',
        message: err instanceof Error ? err.message : 'Failed to update licensing tracker.',
      });
    } finally {
  ...
  const addNote = useCallback(async (userId: number, text: string) => {
    ...
    try {
      const created = await createTrackerNote(userId, normalized, 'licensing');
  ...
  const columns = useMemo(() => buildLicensingColumns({
    onToggle: (userId, field, value) => void handlePatchField(userId, field, value),
  ...
  return (
    <Modal
      open={open}
      title={`Licensing Tracker - ${ownerName}`}
      onClose={onClose}
