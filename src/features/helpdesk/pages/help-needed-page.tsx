    ...
    <div className="min-h-screen bg-[#0b0d12] text-white p-6">
      <div className="mx-auto max-w-6xl">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-[#f5d66a]">Help Needed?</h1>
            <p className="mt-2 text-sm text-white/70">
              Submit a support ticket even if you cannot sign in, then track status by ticket number + email.
            </p>
          </div>
          {isAuthenticated ? (
            <Link to="/home" className="rounded-lg border border-white/20 px-4 py-2 text-sm hover:bg-white/10">
              Back To Dashboard
            </Link>
          ) : (
            <Link to="/login" className="rounded-lg border border-white/20 px-4 py-2 text-sm hover:bg-white/10">
              Back To Login
            </Link>
          )}
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <section className="rounded-2xl border border-white/10 bg-white/5 p-5">
            ...
            {createdTicketNumber && (
              <div className="mt-4 rounded-lg border border-emerald-300/30 bg-emerald-500/10 p-3 text-sm">
          ...
          <section className="rounded-2xl border border-white/10 bg-white/5 p-5">
            ...
            <form className="grid gap-3" onSubmit={handleLookup}>
              ...
              <button
                type="submit"
                className="rounded-lg border border-[#f5d66a]/40 px-4 py-2 text-sm font-semibold text-[#f5d66a] hover:bg-[#f5d66a]/10 disabled:cursor-not-allowed disabled:opacity-60"
                disabled={lookupLoading}
              >
            ...
            {lookupResult && (
              <div className="mt-4 space-y-3 rounded-lg border border-white/15 bg-black/30 p-4 text-sm">
                ...
                <div>
                  ...
                  {lookupResult.comments.length === 0 ? (
                    <p className="text-white/60">No public comments yet.</p>
                  ) : (
                    <div className="space-y-2">
                      {lookupResult.comments.map((comment) => (
                        <div key={comment.id} className="rounded-md border border-white/10 bg-white/5 p-2">
